...
static void sub2video_update(InputStream *ist, AVSubtitle *sub)
{
    ...
    int64_t pts = av_rescale_q(sub->pts, temp_AV_TIME_BASE_Q, ist->st->time_base);
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE)
	{
		AVRational temp_1_sample_rate = {1,ist->st->codec->sample_rate};
...
static int output_packet(InputStream *ist, const AVPacket *pkt)
{
    ...
    if (pkt->dts != AV_NOPTS_VALUE) {
        ...
        if (ist->st->codec->codec_type != AVMEDIA_TYPE_VIDEO || !ist->decoding_needed)
            ist->next_pts = ist->pts = av_rescale_q(pkt->dts, ist->st->time_base, temp_AV_TIME_BASE_Q);
    }
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        ost->forced_kf_pts[i] = av_rescale_q(t, temp_AV_TIME_BASE_Q, avctx->time_base);
...
static int process_input(int file_index)
{
    ...
    if(!ist->wrap_correction_done && input_files[file_index]->ctx->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        int64_t stime = av_rescale_q(input_files[file_index]->ctx->start_time, temp_AV_TIME_BASE_Q, ist->st->time_base);
    ...
    if (debug_ts) {
		char next_dts[AV_TS_MAX_STRING_SIZE],next_dts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(next_dts,ist->next_dts);av_ts_make_time_string(next_dts_time,ist->next_dts,&temp_AV_TIME_BASE_Q);
		char next_pts[AV_TS_MAX_STRING_SIZE],next_pts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(next_pts,ist->next_pts);av_ts_make_time_string(next_pts_time,ist->next_pts,&temp_AV_TIME_BASE_Q);
		char pkt_pts[AV_TS_MAX_STRING_SIZE],pkt_pts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(pkt_pts,pkt.pts);av_ts_make_time_string(pkt_pts_time,pkt.pts,&ist->st->time_base);
		char pkt_dts[AV_TS_MAX_STRING_SIZE],pkt_dts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(pkt_dts,pkt.dts);av_ts_make_time_string(pkt_dts_time,pkt.dts,&ist->st->time_base);
        av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
                ...
                next_dts, next_dts_time,
                next_pts, next_pts_time,
                pkt_pts, pkt_pts_time,
                pkt_dts, pkt_dts_time,
