...
#if HAVE_PTHREADS
/* signal to input threads that they should extra_exit; set by the main thread */
...
static void sub2video_update(InputStream *ist, AVSubtitle *sub)
{
    ...
    if (sub) {
        pts       = av_rescale_q(sub->pts + sub->start_display_time * 1000,
                                 temp_AV_TIME_BASE_Q, ist->st->time_base);
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (debug_ts) {
		char pkt_pts[AV_TS_MAX_STRING_SIZE],pkt_pts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(pkt_pts,pkt->pts);av_ts_make_time_string(pkt_pts_time,pkt->pts,&ost->st->time_base);
		char pkt_dts[AV_TS_MAX_STRING_SIZE],pkt_dts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(pkt_dts,pkt->dts);av_ts_make_time_string(pkt_dts_time,pkt->dts,&ost->st->time_base);
...
static void close_output_stream(OutputStream *ost)
{
    ...
    ost->finished = 1;
    if (of->shortest) {
        int64_t end = av_rescale_q(ost->sync_opts - ost->first_pts, ost->st->codec->time_base, temp_AV_TIME_BASE_Q);
                            ...
                            AVSubtitle *sub)
{
    ...
    for (i = 0; i < nb; i++) {
        ost->sync_opts = av_rescale_q(pts, temp_AV_TIME_BASE_Q, enc->time_base);
        if (!check_recording_time(ost))
            return;

        sub->pts = pts;
        // start_display_time is required to be 0
		AVRational temp_1_1000 = {1, 1000};
        sub->pts               += av_rescale_q(sub->start_display_time, temp_1_1000, temp_AV_TIME_BASE_Q);
        ...
        pkt.pts  = av_rescale_q(sub->pts, temp_AV_TIME_BASE_Q, ost->st->time_base);
        pkt.duration = av_rescale_q(sub->end_display_time, temp_1_1000, ost->st->time_base);
        if (enc->codec_id == AV_CODEC_ID_DVB_SUBTITLE) {
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (qp_hist) {
                ...
                if (qp >= 0 && qp < FF_ARRAY_ELEMS(qp_histogram))
                    qp_histogram[qp]++;
                for (j = 0; j < 32; j++)
                    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%X", (int)lrintf(log2((double)(qp_histogram[j] + 1))));
            }
        ...
        if ((is_last_report || !ost->finished) && ost->st->pts.val != AV_NOPTS_VALUE)
            pts = FFMAX(pts, av_rescale_q(ost->st->pts.val,
                                          ost->st->time_base, temp_AV_TIME_BASE_Q));
    }
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
	AVRational temp_AV_TIME_BASE_Q = {1, AV_TIME_BASE};
    OutputFile *of = output_files[ost->file_index];
    int64_t ost_tb_start_time = av_rescale_q(of->start_time, temp_AV_TIME_BASE_Q, ost->st->time_base);
    AVPicture pict;
    AVPacket opkt;
    ...
    if (pkt->dts == AV_NOPTS_VALUE)
        opkt.dts = av_rescale_q(ist->dts, temp_AV_TIME_BASE_Q, ost->st->time_base);
    else
    ...
    if (ost->st->codec->codec_type == AVMEDIA_TYPE_AUDIO && pkt->dts != AV_NOPTS_VALUE) {
		...
		AVRational tAVRational;
        if(!duration)
            duration = ist->st->codec->frame_size;
		tAVRational.num = 1;
		tAVRational.den = ist->st->codec->sample_rate;
        opkt.dts = opkt.pts = av_rescale_delta(ist->st->time_base, pkt->dts,
                                               tAVRational, duration, &ist->filter_in_rescale_delta_last,
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE) {
        ist->dts = ist->next_dts = ist->pts = ist->next_pts = av_rescale_q(decoded_frame->pts, avctx->time_base, temp_AV_TIME_BASE_Q);
        ...
        decoded_frame_tb   = temp_AV_TIME_BASE_Q;
    }
    if (decoded_frame->pts != AV_NOPTS_VALUE)
	{
		AVRational tAVRational = {1,ist->st->codec->sample_rate};
        decoded_frame->pts = av_rescale_delta(decoded_frame_tb, decoded_frame->pts,
                                              tAVRational, decoded_frame->nb_samples, &ist->filter_in_rescale_delta_last,
                                              tAVRational);
	}
    for (i = 0; i < ist->nb_filters; i++)
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (!ist->decoded_frame && !(ist->decoded_frame = avcodec_alloc_frame()))
        return AVERROR(ENOMEM);
    decoded_frame = ist->decoded_frame;
    pkt->dts  = av_rescale_q(ist->dts, temp_AV_TIME_BASE_Q, ist->st->time_base);
    ...
    if(best_effort_timestamp != AV_NOPTS_VALUE)
        ist->next_pts = ist->pts = av_rescale_q(decoded_frame->pts = best_effort_timestamp, ist->st->time_base, temp_AV_TIME_BASE_Q);

    if (debug_ts) {
		char frame_pts[AV_TS_MAX_STRING_SIZE],frame_pts_time[AV_TS_MAX_STRING_SIZE],best_effort_ts_time[AV_TS_MAX_STRING_SIZE];
		av_ts_make_string(frame_pts,decoded_frame->pts);
		av_ts_make_time_string(frame_pts_time,decoded_frame->pts,&ist->st->time_base);
		av_ts_make_time_string(best_effort_ts_time,best_effort_timestamp,&ist->st->time_base);
        av_log(NULL, AV_LOG_INFO, "decoder -> ist_index:%d type:video "
                "frame_pts:%s frame_pts_time:%s best_effort_ts:%"PRId64" best_effort_ts_time:%s keyframe:%d frame_type:%d \n",
                ist->st->index, frame_pts,
                frame_pts_time,
...
static int output_packet(InputStream *ist, const AVPacket *pkt)
{
    ...
    if (!ist->saw_first_ts) {
        ...
        if (pkt != NULL && pkt->pts != AV_NOPTS_VALUE && !ist->decoding_needed) {
            ist->dts += av_rescale_q(pkt->pts, ist->st->time_base, temp_AV_TIME_BASE_Q);
    ...
    if (pkt->dts != AV_NOPTS_VALUE) {
        ist->next_dts = ist->dts = av_rescale_q(pkt->dts, ist->st->time_base, temp_AV_TIME_BASE_Q);
    ...
    handle_eof:
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (avpkt.duration) {
                duration = av_rescale_q(avpkt.duration, ist->st->time_base, temp_AV_TIME_BASE_Q);
    ...
    if (!ist->decoding_needed) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            if (pkt->duration) {
                ist->next_dts += av_rescale_q(pkt->duration, ist->st->time_base, temp_AV_TIME_BASE_Q);
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    for (p = kf; *p; p++)
        if (*p == ',')
            n++;
    size = n;
    pts = (int64_t *)av_malloc(sizeof(*pts) * size);
    if (!pts) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!memcmp(p, "chapters", 8)) {
            ...
            t = av_rescale_q(t, temp_AV_TIME_BASE_Q, avctx->time_base);
            ...
            av_assert1(index < size);
            pts[index++] = av_rescale_q(t, temp_AV_TIME_BASE_Q, avctx->time_base);
...
static void report_new_stream(int input_index, AVPacket *pkt)
{
    ...
    if (pkt->stream_index < file->nb_streams_warn)
        return;
	char dts_time[AV_TS_MAX_STRING_SIZE];
	av_ts_make_time_string(dts_time,pkt->dts,&st->time_base);
        ...
        if (ost->stream_copy) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!codec->sample_aspect_ratio.num) {
					AVRational temp_0_1 = {0, 1};
                    codec->sample_aspect_ratio   =
                    ost->st->sample_aspect_ratio =
                        ist->st->sample_aspect_ratio.num ? ist->st->sample_aspect_ratio :
                        ist->st->codec->sample_aspect_ratio.num ?
                        ist->st->codec->sample_aspect_ratio : temp_0_1;
                }
            ...
            if (codec->codec_type == AVMEDIA_TYPE_VIDEO) {
                ...
                if (ist && !ost->frame_rate.num)
				{
					AVRational temp_AVRational_25_1 = {25,1};
                    ost->frame_rate = ist->st->r_frame_rate.num ? ist->st->r_frame_rate : temp_AVRational_25_1;
				}
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
		char t_str_error[AV_ERROR_MAX_STRING_SIZE*2] = {0};
        av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
               "next_dts:%s next_dts_time:%s next_pts:%s next_pts_time:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
               ifile->ist_index + pkt.stream_index, av_get_media_type_string(ist->st->codec->codec_type),
               av_ts_make_string(t_str_error,ist->next_dts), av_ts_make_time_string(t_str_error,ist->next_dts, &temp_AV_TIME_BASE_Q),
               av_ts_make_string(t_str_error,ist->next_pts), av_ts_make_time_string(t_str_error,ist->next_pts, &temp_AV_TIME_BASE_Q),
               av_ts_make_string(t_str_error,pkt.pts), av_ts_make_time_string(t_str_error,pkt.pts, &ist->st->time_base),
               av_ts_make_string(t_str_error,pkt.dts), av_ts_make_time_string(t_str_error,pkt.dts, &ist->st->time_base),
               av_ts_make_string(t_str_error,input_files[ist->file_index]->ts_offset),
               av_ts_make_time_string(t_str_error,input_files[ist->file_index]->ts_offset, &temp_AV_TIME_BASE_Q));
    }
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            for (i=0; i<is->nb_streams; i++) {
                ...
                if(st->discard == AVDISCARD_ALL || st->start_time == AV_NOPTS_VALUE)
                    continue;
                new_start_time = FFMIN(new_start_time, av_rescale_q(st->start_time, st->time_base, temp_AV_TIME_BASE_Q));
            }
        ...
        stime = av_rescale_q(is->start_time, temp_AV_TIME_BASE_Q, ist->st->time_base);
    ...
    if (pkt.dts != AV_NOPTS_VALUE)
        pkt.dts += av_rescale_q(ifile->ts_offset, temp_AV_TIME_BASE_Q, ist->st->time_base);
    if (pkt.pts != AV_NOPTS_VALUE)
        pkt.pts += av_rescale_q(ifile->ts_offset, temp_AV_TIME_BASE_Q, ist->st->time_base);
    ...
    if (pkt.dts != AV_NOPTS_VALUE && ist->next_dts != AV_NOPTS_VALUE &&
        !copy_ts) {
        int64_t pkt_dts = av_rescale_q(pkt.dts, ist->st->time_base, temp_AV_TIME_BASE_Q);
        ...
        if(delta < -1LL*dts_delta_threshold*AV_TIME_BASE ||
            ...
            pkt.dts -= av_rescale_q(delta, temp_AV_TIME_BASE_Q, ist->st->time_base);
            if (pkt.pts != AV_NOPTS_VALUE)
                pkt.pts -= av_rescale_q(delta, temp_AV_TIME_BASE_Q, ist->st->time_base);
        }
        } else {
            ...
            if (pkt.pts != AV_NOPTS_VALUE){
                int64_t pkt_pts = av_rescale_q(pkt.pts, ist->st->time_base, temp_AV_TIME_BASE_Q);
...
//-i "D:/AVTestFile/AVNormal/°®ÇéÎåÏßÆ×.ts" -vcodec x264Encoder -profile:v main -b:v 262144 -s 480x320 -acodec libmp3lame -copyts -y "D:/AVTestFile/OutPut/jingyizhilv.avfs"
