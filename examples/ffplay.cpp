...
static enum ShowMode show_mode = ShowMode::SHOW_MODE_NONE;
            ...
            if (!is->audio_buf_frames_pending) {
                ...
                reconfigure =
                    cmp_audio_fmts(is->audio_filter_src.fmt, is->audio_filter_src.channels,
                                   (AVSampleFormat)is->frame->format, av_frame_get_channels(is->frame))    ||
                ...
                if (reconfigure) {
                    ...
                    av_log(NULL, AV_LOG_DEBUG,
                           ...
                           is->frame->sample_rate, av_frame_get_channels(is->frame), av_get_sample_fmt_name((AVSampleFormat)is->frame->format), buf2, is->audio_pkt_temp_serial);

                    is->audio_filter_src.fmt            = (AVSampleFormat)is->frame->format;
...
//      of the seek_pos/seek_rel variables
 ...
 fail:
    ...
    if (ret != 0) {
        ...
        event_SDL.type = FF_QUIT_EVENT;
        event_SDL.user.data1 = is;
    ...
    for (;;) {
        ...
        case SDL_MOUSEMOTION:
            ...
            if (event_SDL.type == SDL_MOUSEBUTTONDOWN) {
                x = event_SDL.button.x;
            } else {
                if (event_SDL.motion.state != SDL_PRESSED)
                    break;
                x = event_SDL.motion.x;
            }
        ...
        case SDL_VIDEORESIZE:
                ...
                screen_width  = cur_stream->width  = event_SDL.resize.w;
                screen_height = cur_stream->height = event_SDL.resize.h;
                cur_stream->force_refresh = 1;
            break;
        case SDL_QUIT:
        case FF_QUIT_EVENT:
...
#include "cmdutils_common_opts.h"
    ...
    { "sn", OPT_BOOL, { (void*)&subtitle_disable }, "disable subtitling" },
